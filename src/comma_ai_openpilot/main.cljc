(ns comma_ai_openpilot.main
  "Kotodama WASM entrypoint for the Comma_ai_openpilot clean-room actor (L4) — Clojure port.
  ...
  facts (`comma_ai_openpilot.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "comma_ai_openpilot")
(def tier "L4")
...
(def entity-specs
  [{:entity "Event"      :plural "events"      :id-prefix "commaaio_eve"
    :fields [:logMonoTime :valid :logMessage] :required [:logMonoTime :valid]
    :coerce {:logMonoTime :int}   :refs {}}
   {:entity "CarState"   :plural "carstates"   :id-prefix "commaaio_car"
    :fields [:vEgo :aEgo :standstill :gasPressed :brakePressed :steeringAngleDeg :steeringTorque :gearShifter :leftBlinker :rightBlinker] :required [:vEgo :aEgo]
    :coerce {:vEgo :float :aEgo :float :steeringAngleDeg :float :steeringTorque :float} :refs {}}
   {:entity "CarControl" :plural "carcontrols" :id-prefix "commaaio_car"
    :fields [:enabled :latActive :longActive :leftBlinker :rightBlinker] :required [:enabled :latActive]
    :coerce {:enabled :bool :latActive :bool :longActive :bool :leftBlinker :bool :rightBlinker :bool} :refs {}}
   {:entity "CarParams"  :plural "carparamses" :id-prefix "commaaio_car"
    :fields [:brand :carFingerprint :minEnableSpeed :minSteerSpeed :mass :wheelbase] :required [:brand :carFingerprint]
    :coerce {:minEnableSpeed :float :minSteerSpeed :float :mass :float :wheelbase :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"comma_ai_openpilot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "comma_ai_openpilot-compat" :tier tier :entities entities} 200])
...
(defn start! [] :comma_ai_openpilot-compat/ready)
