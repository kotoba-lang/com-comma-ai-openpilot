(ns comma_ai_openpilot.main-test
  "Contract + behavioral test for the comma_ai_openpilot-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "comma_ai_openpilot-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'comma_ai_openpilot.main-test)]
